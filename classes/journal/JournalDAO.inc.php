...
class JournalDAO extends DAO {
	...
	function insertJournal(&$journal) {
		$this->update(
			'INSERT INTO journals
				(path, seq, enabled, active, primary_locale)
				VALUES
				(?, ?, ?, ?, ?)',
			array(
				$journal->getPath(),
	...
	function &getJournals($enabledOnly = false, $rangeInfo = null, $sortBy = JOURNAL_FIELD_SEQUENCE, $searchField = null, $searchMatch = null, $search = null) {
		...
		if ($enabledOnly) $whereSql .= ($whereSql?'AND ':'') . 'j.enabled=1 AND j.active=1';
	...
	/**
	 * New Function added so i can show just the disabled journals
	 */
	 
	 function &getOldJournals($rangeInfo = null, $sortBy = JOURNAL_FIELD_SEQUENCE, $searchField = null, $searchMatch = null, $search = null) {
		$joinSql = $whereSql = $orderBySql = '';
		$params = array();
		$needTitleJoin = false;
		...
		// Handle filtering conditions
		$whereSql .= ($whereSql?'AND ':'') . 'j.enabled=1 AND j.active=0';
